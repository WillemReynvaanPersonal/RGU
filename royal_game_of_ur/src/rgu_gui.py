...
for h in range(3):
    for w in range(8):
        square_rect[h].append(
            pygame.Rect(
                margin_left + square_width * w,
...
while 1:
    ...
    if rgu.check_winner() == -1:
        roll_label = game_font.render(
            "Player {} rolls: {}".format(rgu.player, roll), 1, black
        )
    else:
        roll_label = game_font.render(
            "Player {} has won".format(rgu.check_winner()), 1, black
        )
        gamestate = 3
